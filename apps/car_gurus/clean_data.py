...
def get_result_pages():
    ...
    driver = webdriver.Chrome(
        executable_path="C:\\chromedriver-win64\\chromedriver.exe", options=options
    )
    ...
    input_box = driver.find_element(
        By.XPATH, "//*[@id='dealFinderZipUsedId_dealFinderForm']"
    )
...
def scrape_listings(listing_urls):
    ...
    for j in range(1003):
        ...
        for i in range(len(feature_keys)):
            ...
            if (
                feature_keys[i].text == "NHTSA rollover rating:"
                or feature_keys[i].text == "NHTSA side crash rating:"
                or feature_keys[i].text == "NHTSA frontal crash rating:"
                or feature_keys[i].text == "NHTSA overall safety rating:"
            ):
                continue
